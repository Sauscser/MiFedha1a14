  ...
  const handleSignatory3 = async () => {
    ...
    try {
      ...
      const groupData: any = await API.graphql(
        graphqlOperation(getGroup, { grpContact: member.groupContact })
      );
      ...
      const receiverData: any = await API.graphql(
        graphqlOperation(getSMAccount, { awsemail: member.memberContact })
      );
      const receiver = receiverData.data.getSMAccount;

      const bankAdminData: any = await API.graphql(
        graphqlOperation(getMiFedhaBankAdmin, { nationalid: group.BankAdminAcNu })
      );
      const bankAdmin = bankAdminData.data.getMiFedhaBankAdmin;

      const controlData: any = await API.graphql(
      ...
      // Update confirmation
      await API.graphql(graphqlOperation(updateGroupNonLoans, { input: { id, confirm2: "YES" } }));
      ...
      // Update bank admin
      await API.graphql(graphqlOperation(updateMiFedhaBankAdmin, {
        input: {
      ...
      await API.graphql(graphqlOperation(updateChamaControlTable, {
        input: {
          id: "EQUITYTABLEID",
